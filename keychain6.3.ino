...
const char *LOCAL_SSID = "Lena2";
const char *LOCAL_PASS  = "LenaLenaLena";
...
display.println("Dining Buddy v6.3");
display.println("I'm waking up...");
display.display();

  if (!SPIFFS.begin(true)) {
    showProgress("Sorry! :( SPIFFS failed!");
    while (1);
  }
  ...
  if(wifiFailed){
    ...
    // Show multi-line message
    display.clearDisplay();
    display.setCursor(0, 0);
    display.setTextSize(1);
    display.setTextColor(SSD1306_WHITE);
    showProgress("WiFi failed! Sorry :( Hold button 3s to reboot me!");
    ...
    while (1) {
      ...
      if (buttonPressed && !longPressDetected) {
        ...
        } else if (millis() - pressStartTime > 3000) { // 3 second long press
...
void scheduleDeepSleepUntilNightWake() {
  ...
  if (!getLocalTime(&timeinfo)) {
    ...
    return; // CRITICAL: Add return statement
  }
...
void rotateAndFetchMenus() {
  ...
  for (int h = 0; h < NUM_HALLS; h++) {
    ...
    // Check if 6th-day file exists and has content
    if (SPIFFS.exists(file)) {
      File checkFile = SPIFFS.open(file, FILE_READ);
      if (checkFile && checkFile.size() > 100) { // File exists and has meaningful content
        showProgress(String("Skip +6 ") + halls[h].name);
        checkFile.close();
  ...
  for (int i = 0; i <= numDays; i++) {
    ...
    for (Hall &h : halls) {
      ...
      if (SPIFFS.exists(fileName)) {
        ...
        if (checkFile && checkFile.size() > 100) { // File exists and has meaningful content
          showProgress("Skip " + String(h.name) + " " + dateStr);
          checkFile.close();
          continue; // Skip downloading this file
        }
        checkFile.close();
...
bool connectLocalWiFi(const char *ssid, const char *pwd, unsigned long timeoutMs) {
  showProgress("I'm Connecting to Wi-Fi...");
  WiFi.mode(WIFI_STA);
  WiFi.begin(ssid, pwd);
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    if (millis() - startAttempt > timeoutMs) {
      showProgress("I searched for 30s but couldn't connect to Wi-Fi!");
      return false;
    }
  }
...
bool connectEduroam(unsigned long timeoutMs) {
  showProgress("I'm Connecting eduroam...");
  WiFi.mode(WIFI_STA);
...
bool downloadPage(const char *host, const char *path) {
  ...
  showProgress("I'm downloading https://" + String(host) + String(path));
...
void showCurrentPage() {
  ...
  // Only add meal letter if not Rendezvous
  if (page.mealLetter.length() > 0 && String(halls[currentHallIndex].name) != "Rendezvous") {
